...
AUTO_MIN_LIQUIDITY_USDT = Decimal("5000000")
...
class _CandidateEvidence:
    """Rolling, independent profitable observations for one AUTO symbol."""
...
class ArbitragePaperService:
    ...
    def _record_auto_candidate_evidence(
        ...
        *,
        ...
        """Record at most one independent profitable snapshot per symbol.

        Re-reading the same BBO cannot turn one transient signal into several
        qualifying observations.  The required count and rolling window are
        supplied by the current AUTO settings.
        """
        ...
        for symbol, opportunity in best_by_symbol.items():
            ...
            evidence.best_net_edge_bps = max(evidence.best_net_edge_bps, edge_bps)
            ...
            if active is not None and active.state != "pending_liquidation":
                active.last_signal_ms = now_ms
                active.state = "active"
        return set(best_by_symbol)
    ...
    def _activation_plan(
        ...
        for (candidate_symbol, venue), book in sorted(books.items()):
            ...
            # Allocation is skipped, rather than partially filled, when the
            # public BBO cannot absorb 20x one capped PAPER trade.  This is a
            # conservative anti-illiquidity gate, not a promise of live fill.
            required_bbo_notional = settings.notional * Decimal("20")
            if (
    ...
    def _liquidate_expired_auto_inventory(
        ...
        *,
        ...
        for symbol, active in tuple(self._active_inventory.items()):
            if (
                active.state != "pending_liquidation"
                and now_ms - active.last_signal_ms
                <= settings.evidence_window_ms
            ):
                continue
            active.state = "pending_liquidation"
            deltas: dict[tuple[str, str], Decimal] = {}
    ...
    async def _collect_auto_books(
        ...
        self._universe = select_cross_venue_universe(
            ...
            quote_asset="USDT",
            min_venues=2,
            min_liquidity_usdt=AUTO_MIN_LIQUIDITY_USDT,
        )
        selected_venues = {
        ...
        *,
        ...
        async with self._scan_lock:
            ...
            try:
                ...
                engine = ArbitrageEngine(self._config(selected, symbols))
                ...
                if selected.symbol == AUTO_SYMBOL and self._auto_inventory_enabled:
                    # Discovery intentionally ignores balances.  It observes
                    # all 50 candidates without buying them, records unique
                    # profitable snapshots, and only then allocates confirmed
                    # inventory before the balance-aware execution rescan.
                    ...
                    # The UI continues to show a candidate signal while it is
                    # collecting evidence, even though it is not executable
                    # until inventory has been allocated.
            ...
            rows.append(
                {
                    ...
                    "last_signal_ms": active.last_signal_ms,
        ...
        return {
            ...
            "market_data_policy": {
                ...
                "auto_paper_inventory": (
                    "balance-conserving USDT-to-base PAPER allocations after "
                    "the configured number of distinct profitable snapshots "
                    "inside the configured rolling window"
                ),
                ...
                "auto_activation_min_bbo_notional_usdt": decimal_string(
                    self._settings.notional * Decimal("20")
                ),
                "auto_expiry": (
                    "sell base at executable bid after the configured evidence "
                    "window without a qualifying signal; retain slot while "
