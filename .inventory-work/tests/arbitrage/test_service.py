...
async def test_auto_uses_custom_activation_trade_and_allocation_settings() -> None:
    ...
    assert policy["auto_activation_min_bbo_notional_usdt"] == "600"
...
async def test_auto_activates_at_most_two_symbols() -> None:
    ...
    ccc = next(
        row for row in result["candidate_evidence"] if row["symbol"] == "CCCUSDT"
    )
    assert ccc["ready"] is True
    assert ccc["active"] is False
    assert result["metrics"]["trade_count"] == 1
...
async def test_auto_skips_allocation_without_full_bbo_capacity() -> None:
    ...
    assert result["candidate_evidence"][0]["ready"] is True
    assert result["active_inventory"] == []
    assert result["metrics"]["trade_count"] == 0
...
@pytest.mark.asyncio
async def test_auto_uses_configured_evidence_window_for_expiry() -> None:
    current_ms = [NOW_MS]
    ...
    service = ArbitragePaperService(
        (
            SequencedAutoAdapter("cheap", ((signal("cheap"),), (calm("cheap"),))),
            SequencedAutoAdapter("rich", ((signal("rich"),), (calm("rich"),))),
        ),
        clock_ms=lambda: current_ms[0],
    )
    settings = ScanSettings(
