...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("adapter_type", "expected_path", "expected_params", "payload", "venue"),
    [
        (
            HuobiPublicAdapter,
            "/market/depth",
            {"symbol": "btcusdt", "type": "step0", "depth": "5"},
            {
                "status": "ok",
                "ts": 1_723_456_789_000,
                "tick": {
                    "ts": 1_723_456_789_010,
                    "version": 42,
                    "bids": [[100, 2]],
                    "asks": [[101, 3]],
                },
...
async def test_new_public_adapters_normalize_spot_order_books(
    adapter_type: type,
    expected_path: str,
    ...
    async with _client(handler) as client:
        adapter = adapter_type(
            ...
            clock_ms=lambda: 1_723_456_789_000,
        )
        book = await adapter.fetch_order_book("btc/usdt", depth=2)
    ...
    assert book.symbol == "BTCUSDT"
    assert book.bids[0] == type(book.bids[0])(
        price=Decimal("100"), quantity=Decimal("2")
    )
...
@pytest.mark.asyncio
async def test_huobi_alias_and_supported_depth_round_up() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.params["depth"] == "10"
        return httpx.Response(
            200,
            json={
                "status": "ok",
    ...
    assert HTXPublicAdapter is HuobiPublicAdapter
    async with _client(handler) as client:
        adapter = HTXPublicAdapter(client=client, base_url="https://mock.test")
        await adapter.fetch_order_book("BTCUSDT", depth=6)
...
async def test_new_adapters_reject_locked_order_books(adapter_type: type) -> None:
    if adapter_type is HuobiPublicAdapter:
        payload = {
            "status": "ok",
            "ts": 1_723_456_789_000,
            "tick": {"bids": [[101, 1]], "asks": [[101, 1]]},
        }
    elif adapter_type is KuCoinPublicAdapter:
        payload = {
            "code": "200000",
            "data": {
    ...
    elif adapter_type is BingXPublicAdapter:
        payload = {
            "code": 0,
            "data": {
                "ts": 1_723_456_789_000,
    ...
    async with _client(lambda _request: httpx.Response(200, json=payload)) as client:
        ...
        with pytest.raises(OrderBookPayloadError, match="locked or crossed"):
            await adapter.fetch_order_book("BTCUSDT")
