...
**Дата:** 2026-08-11<br>
**Рынок:** Bybit V5, `category=linear`<br>
**Основные инструменты:** `BTCUSDT`, `ETHUSDT`, `XRPUSDT`<br>
**Режим эксплуатации:** один сервер, 24/7<br>
**Адресаты:** тимлид, backend/frontend/ML-инженеры, QA/SRE<br>
**Статус:** целевая архитектура и порядок внедрения

**Изменения v1.1:** provider-neutral LLM layer, Strategy Research Sandbox,
границы OpenAI/Anthropic/DeepSeek credentials и воспроизводимость
...
### 16.1.1. Provider-neutral LLM layer

LLM Assistant зависит только от внутреннего versioned-контракта
`LLMProvider`. Provider и точный model ID выбираются конфигурацией; OpenAI,
Anthropic, Google, Mistral/DeepSeek и локальный inference подключаются
отдельными adapters. Выбор primary и A/B/backup provider утверждается в
ADR-010 после проектных evals; доменная логика от конкретного SDK не зависит.

```text
ModelRequest(messages, tools, responseSchema, deadlineMs, traceId)
ModelResult(content, toolCalls, structuredJson, provider, modelId,
            providerRequestId, usage)
```

Контракт нормализует structured output по JSON Schema, tool-call requests,
timeout/cancellation/retry, rate limits, usage/cost и provider request ID.
Модель не исполняет tool сама: она формирует `ToolCallRequest`, а permission
gateway проверяет principal, allowlist, schema и budget и только затем
выполняет вызов. Provider-specific payload не выходит за adapter.

Каждый answer и research artifact хранит provider, точный model ID/snapshot,
prompt version, tool/schema versions, generation parameters, provider request
ID и usage. Скрытый fallback запрещён для strategy research: повтор у другого
provider создаёт отдельный artifact с новым provenance и проходит валидацию
заново. Hosted provider получает только sanitized bounded context; raw history,
Bybit credentials, private account data и execution state ему не передаются.
...
### 16.3.1. Strategy Research Sandbox

Новые гипотезы описываются декларативным versioned `StrategyCandidate`
(JSON/DSL), а не произвольным Python. Контракт содержит allowed
features/regimes, causal entry/cancel conditions, entry/invalidation, SL/TP,
logic/time stop, maximum holding, quality policy и bounded parameter ranges.

```text
LLM hypothesis → StrategyCandidate → schema/type/causality validation
→ approved immutable JobSpec → deterministic replay/backtest
→ walk-forward → frozen candidate → sealed OOS
→ immutable report + complete trial ledger → human review
→ отдельная production implementation/config revision
→ signal-only → paper → demo → canary
```

Sandbox запускается как отдельная bounded job: approved datasets доступны
read-only; нет Bybit/execution socket, API keys и outbound network по
умолчанию; запись разрешена только во временный каталог и versioned research
artifacts. Ограничиваются CPU/RAM/disk/wall-time, число trials и API budget;
admission controller может pause/cancel job. Используется общий
simulator/replay engine, а не второй упрощённый backtester.

Registry сохраняет candidate revision, dataset manifest hashes и gap map, code
SHA, config/runtime versions, seed, split, все trials, включая неудачные, и
checksum отчёта. Результат имеет только `DRAFT` status и не может менять active
config, merge code, promote model/strategy или создавать `OrderIntentProposal`.
Arbitrary generated code в v1 запрещён; его допуск требует отдельного ADR,
усиленной OS-изоляции и sandbox-escape tests.
...
| Create draft StrategyCandidate | Только новая immutable revision по schema |
| Execute arbitrary generated code | Запрещено в v1 |
| Write active strategy/runtime config | Всегда запрещено |
| Network access из research sandbox | Запрещено по умолчанию |
...
#### 16.5.1. Credentials провайдеров

Формат ключа считается opaque secret: приложение не валидирует и не логирует
его префикс. В git, versioned config, prompts, tool payloads и artifacts
хранятся только secret references.

```text
# Native OpenAI API: HTTP Authorization: Bearer
OPENAI_API_KEY=<secret-ref>
# OPENAI_BASE_URL=<approved-gateway-url>  # только для явного proxy/gateway

# Native Anthropic API: HTTP X-Api-Key
ANTHROPIC_API_KEY=<secret-ref>

# Native DeepSeek API: OpenAI-compatible, HTTP Authorization: Bearer
DEEPSEEK_API_KEY=<secret-ref>
DEEPSEEK_BASE_URL=https://api.deepseek.com

# Anthropic-compatible gateway с Bearer, если таков контракт gateway
ANTHROPIC_BASE_URL=https://gateway.example.invalid
ANTHROPIC_AUTH_TOKEN=<secret-ref>

# DeepSeek через Claude Code, только для developer workstation
# ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic
# ANTHROPIC_AUTH_TOKEN=<separate-deepseek-secret-ref>
```

Для Anthropic gateway выбирается ровно один auth mode:
`ANTHROPIC_API_KEY`/`X-Api-Key` либо `ANTHROPIC_AUTH_TOKEN`/Bearer. Одновременное
наличие обоих credentials — configuration error и fail-closed. Переменная
`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` настраивает клиент Claude Code и не
является настройкой backend adapter платформы.

Server-side DeepSeek adapter использует отдельный `DEEPSEEK_API_KEY` и
OpenAI-compatible protocol; он не переиспользует `OPENAI_API_KEY` или
Claude Code token. Model ID и capability matrix (`thinking`,
`reasoning_content`, tools, structured output) являются versioned config и
проходят тот же adapter contract/eval suite. Development, staging и production
используют разные provider credentials.

### 16.6. Приёмка

- AI answer содержит data/config/model versions.
- Все provider adapters проходят единый contract/eval suite; скрытого fallback
  нет.
- Backtest request воспроизводится по ID.
- Нет leakage и training/test overlap.
- Sandbox не читает secrets, не имеет Bybit/execution/network access и не может
  изменить active config.
- Report содержит immutable JobSpec и полный ledger всех trials.
...
- Bybit API key без withdrawal permission.
- IP whitelist, если поддерживается окружением.
- Отдельные Bybit keys для demo/testnet/live.
- Secrets вне git, versioned config/config snapshots, prompts, tool payloads,
  artifacts, logs и UI.
- Bybit credentials доступны только `execution-risk`. LLM credentials доступны
  только server-side provider adapter; LLM context, permission tools и Strategy
  Research Sandbox не могут их читать.
- Provider/model/base URL — non-secret config. Credential подаётся deploy-time
  из secret manager или root-readable `EnvironmentFile`, не через CLI.
...
Реализовать versioned `LLMProvider` contract и adapters, assistant read tools,
job queue, optimizer, registry, затем ML baseline. Новые гипотезы запускаются
через typed `StrategyCandidate` и изолированный bounded Research Sandbox.
Не начинать с arbitrary generated code, deep/RL или доступа к live execution.

Приёмка: adapter conformance/evals, lineage, leakage tests, complete trial
ledger, reproducible runs, sandbox/secret/network isolation, permissions,
abstention, cancellation без влияния на live и human promotion.
